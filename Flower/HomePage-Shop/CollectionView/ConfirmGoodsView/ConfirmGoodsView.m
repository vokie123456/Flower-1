...
@implementation ConfirmGoodsView
...
- (instancetype)initWithGoodName:(NSString *)goodName flowersNumber:(NSString *)number delegate:(id)delegate{
    ...
    self.currentNumber=[manager readingFile];
    // 接收键盘显示隐藏的通知
...
- (void)setUpUI{
    ...
    self.tipLabel.text=[NSString stringWithFormat:@"确定要兑现“%@”的愿望吗，即将扣除%@个小鲜花。",self.goodName,self.number];
    ...
    self.warningLabel=[[UILabel alloc]initWithFrame:CGRectMake(0, self.tipLabel.maxY+20, self.contentView.width, 30)];
    self.warningLabel.text=@"";
    self.warningLabel.font=[UIFont systemFontOfSize:12.f];
...
-(void)confirmButtonClicked:(UIButton *)sender{
    ...
    if(self.number>self.currentNumber){
        self.warningLabel.text=@"当前的小红花余额不足，再接再厉！";
    }
    else{
